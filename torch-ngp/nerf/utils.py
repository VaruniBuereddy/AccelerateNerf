...
class SSIMMeter:
    ...
    def update(self, preds, truths):
        ...
        if len(preds.shape)!=4:
            ...
            truths = torch.unsqueeze(truths, 0)
            print(preds.shape)
            print(truths.shape)
...
class Trainer(object):
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            values = self.train_one_epoch(train_loader)
            print(values)
            self.metric_values[0].append(values[0])
            self.metric_values[1].append(values[1])
            self.metric_values[2].append(values[2])
            ...
            if self.local_rank == 0:
                if self.report_metric_at_train:
                    for i, metric in enumerate(self.metrics):
                        ...
                        # self.values[i]+= metric.measure()/len(loader)
        ...
        if self.local_rank == 0:
            ...
            if self.report_metric_at_train:
                for i, metric in enumerate(self.metrics):
                    ...
                    values.append(metric.measure())
                    if self.use_tensorboardX:
                        metric.write(self.writer, self.epoch, prefix="train")
                    if self.use_wandb and i==0:
                        wandb.log({"psnr": metrics.measure()})
                            # wandb.log({"lr": self.optimizer.param_groups[0]['lr']})
                    metric.clear()
