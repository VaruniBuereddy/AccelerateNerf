...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(render_poses):
        print(f'Progress: {i / len(render_poses)}, time: {time.time() - t}')
        t = time.time()
...
def run(config=None):
    with wandb.init(config=config):
        ...
        args.lrate = config.lrate
        args.chunk = config.chunk
...
def train(args):
    ...
    print(f'args.render_only:{args.render_only}')
...
if __name__=='__main__':
    ...
    sweep_id = wandb.sweep(sweep_config, project="Sweep_Nerf")
