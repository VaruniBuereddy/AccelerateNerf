...
class PSNRMeter:
    ...
    def update(self, preds, truths):
        ...
        print(type(preds), type(preds[0]), type(truths), type(truths[0]))
...
class SSIMMeter:
    ...
    def measure(self):
        return (self.V / self.N).cpu().numpy()
...
class LPIPSMeter:
    ...
    def update(self, preds, truths):
        preds, truths = self.prepare_inputs(preds, truths) # [B, H, W, 3] --> [B, 3, H, W], range in [0, 1]
        print(preds.shape, type(preds), truths.shape, type(truths))
        v = self.fn(truths, preds, normalize=True)
        v = torch.mean(v).item() # normalize=True: [0, 1] to [-1, 1]
        self.V += v
        self.N += 1
